...
25 GB of free space on the host should be enough, but remember a full disk runs slow and it is best to have about 25% of the host disk unused.

## Download Ubuntu 18.04 Desktop LTS
> TODO: recommend using Transmission
...
![do you want to run guest additions](resources/create-ubuntu-vm/do-you-want-to-run-ga.png)
...
> Super key: In Linux the Windows key is called 'super'

Finally, my keyboard has only one Windows key and it is on the right side. Ubuntu expects there to be one on the left side so the onle on the right does nothing. To fix this:
...
In order for shared folders to work you need to add your user to the `vboxsf` group.
